...
export const Doctor_Home_Page=()=>{
    ...
    useEffect(() => {
        const fetch_data = async () => {
            let response = await axios.post("http://localhost:3500/doctor/get_full_doctor_details/", { u_id: sessionStorage.getItem("username") });
            if (response.data.success_status) {
                set_person_info(response.data.ans[0]);
                set_other_info(response.data.ans[1]);

                response = await axios.post("http://localhost:3500/doctor/doctor_view_redirection_records/", { u_id: sessionStorage.getItem("username") });
                if (response.data.success_status) {
        ...
        if (sessionStorage.getItem("is_authenticated") === "true" && sessionStorage.getItem("user_designation") === "Doctor") {
            fetch_data();
        }
        else {
    ...
    return(
        ...
        {unserved_redirection_records.map(record => (
            ...
            <h2 onClick={(e) => navigator("/doctor/show_patient_history")}>Show Patient History</h2>
        </Fragment>
    );
};